...
for year in YEARS:
    ...
    try:
        ...
        individus_per_imatge = list(data.groupby("id_document").count()["id_padro_individu"].values)
        imatges = sorted(glob.glob(actual_year + "/*.jpg"))

        if len(imatges) != len(individus_per_imatge):
            expeced_sequence = (pd.unique(data["id_document"]))
            n = len(expeced_sequence) + 1
            a = min(expeced_sequence)
            l = max(expeced_sequence)

            actual_sum = sum(expeced_sequence)  
            expected_sum = (n * (a + l)) // 2

            missing_document = expected_sum - actual_sum
            index_error = list(expeced_sequence).index(missing_document+1)
            individus_per_imatge.insert(index_error, 0)
        ...
        if len(imatges) == 0:
            ...
            for extra_vol in extra_volumes:
                ...
                with open(v + "graph_gt_corroborator.json", "a") as file:
